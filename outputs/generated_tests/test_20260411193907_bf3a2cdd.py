...
# execution_id: 20260411193907_bf3a2cdd
# generated_at: 2026-04-11T19:39:07.835201
...
def test_1_TEST_CASE_000000006():
    ...
    try:
        ...
        _resp_json = {}
    _biz_code = _resp_json.get("code") if isinstance(_resp_json, dict) else None
    if resp.status_code == 200 and _biz_code is not None and _biz_code != "00000000":
        _fail_msg = (
            "[业务code失败] code=" + str(_biz_code) + ", " +
            "message=" + str(_resp_json.get("message")) + ", " +
            "响应体=" + json.dumps(_resp_json, ensure_ascii=False)
        )
        logger.error("【断言失败】%s", _fail_msg)
        raise AssertionError(_fail_msg)
    logger.info("【业务code检查】code=%s, 状态码=%d, 检查通过", _biz_code, resp.status_code)
