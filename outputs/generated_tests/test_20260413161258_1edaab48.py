...
# execution_id: 20260413161258_1edaab48
# generated_at: 2026-04-13T16:12:58.177423
...
def test_1_TEST_CASE_000000007():
    """
    付款前前置操作

    Case ID: TEST_CASE_000000007
    Priority: P0
    Method: POST /v6/api/cashout/payment/common-payment-bill
    """
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "TEST_CASE_000000007", "计费", "交易模块")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/v6/api/cashout/payment/common-payment-bill", BASE_URL.rstrip("/") + "/" + "/v6/api/cashout/payment/common-payment-bill".lstrip("/"))
    ...
    _raw_body = {"uuid": "b93f27ae252e4ef4a5a77723db2057ef", "sesAmt": 11, "accuntId": 11358, "txnModel": "1", "accountNo": "ff813c70da4a4d5cb3e52fb72ed615d9", "reqSource": "WEB", "chargeType": "SHA", "paymentAmt": 8.59, "paymentWay": "LOCAL", "accCurrency": "USD", "realNameFlag": "1", "deductFeeMode": "OUTER", "paymentCountry": "GB", "paymentWayName": "英国本地FPS", "vaBusinessType": "B2B", "paymentCurrency": "GBP", "localSettleMethod": "FPS", "supplierOrderType": "1"}
    ...
    logger.info("【完整请求】")
    logger.info("    URL: %s", BASE_URL.rstrip("/") + "/" + "/v6/api/cashout/payment/common-payment-bill".lstrip("/"))
    ...
    _request_url = BASE_URL.rstrip("/") + "/" + "/v6/api/cashout/payment/common-payment-bill".lstrip("/")
    ...
    resp = _request(
        method="POST",
        path="/v6/api/cashout/payment/common-payment-bill",
        headers={"user_info": "{\"id\": \"20250625151835001664133640\", \"userPhone\": \"11908312323\", \"userEmail\": \"11908312323@qq.com\", \"userStatus\": \"2\", \"inMno\": \"603250625000506\", \"merchantRegion\": \"HK\", \"countryCode\": \"CN\", \"jhMno\": \"M001200000003196\", \"mecTypeCode\": \"15\", \"userType\": 1, \"subjectArea\": \"HK\", \"merType\": \"00\", \"agentFlag\": false, \"dataIsolation\": 0, \"accountServiceFlag\": \"00\", \"loginSource\": \"PC\"}", "Content-Type": "application/json"},
        params=None,
        json_body={"uuid": "b93f27ae252e4ef4a5a77723db2057ef", "sesAmt": 11, "accuntId": 11358, "txnModel": "1", "accountNo": "ff813c70da4a4d5cb3e52fb72ed615d9", "reqSource": "WEB", "chargeType": "SHA", "paymentAmt": 8.59, "paymentWay": "LOCAL", "accCurrency": "USD", "realNameFlag": "1", "deductFeeMode": "OUTER", "paymentCountry": "GB", "paymentWayName": "英国本地FPS", "vaBusinessType": "B2B", "paymentCurrency": "GBP", "localSettleMethod": "FPS", "supplierOrderType": "1"},
        timeout=30,
    )
    _extract_cfg = [{"name": "billId", "path": "$.data.billId"}]
    logger.info("【字段提取】开始提取响应字段, 配置数=%d: %s", len(_extract_cfg), [f["name"] for f in _extract_cfg])
    _extracted_data = _extract_fields_by_config(resp, _extract_cfg)
    logger.info("【字段提取】提取结果: %s", json.dumps(_extracted_data, ensure_ascii=False))
    _session_ctx = _get_session_context()
    _session_ctx.update(_extracted_data)
    _store_test_context("test_1_TEST_CASE_000000007", resp, _extracted_data)
    ...
    logger.info("【提取字段】配置数=1, 字段名: ['billId']")
    logger.info("    提取配置: billId <- $.data.billId  ()")
    ...
    logger.info("=" * 60)
    logger.info("【断言信息】")
    logger.info("    用例ID: TEST_CASE_000000007")
    logger.info("    断言数量: 0")
