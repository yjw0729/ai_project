...
# execution_id: 20260415222831_08103895
# generated_at: 2026-04-15T22:28:31.846753
...
def test_1_TEST_CASE_000000101():
    """
    productCount为null，应视为缺失或非法，校验失败

    Case ID: TEST_CASE_000000101
    Priority: P0
    Method: POST /businessAction/commonBusiness/BASIC_BZFK
    """
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "TEST_CASE_000000101", "productCount为null（JSON null值）", "交易")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/businessAction/commonBusiness/BASIC_BZFK", BASE_URL.rstrip("/") + "/" + "/businessAction/commonBusiness/BASIC_BZFK".lstrip("/"))
    logger.info("【请求头】无自定义请求头，使用默认Content-Type")
    logger.info("【Query参数】无Query参数")
    _raw_body = {"request": {"body": {"data": {"remark": "aaa", "notifyUrl": "http://mqhjshjkr.bm/ihb", "requestId": "{{outTradeNo}}", "rootMchId": "600600010996559", "orderExpired": "90", "batchActionList": [{"amount": "3", "remark": "bbb", "payeeMno": "600600010996558", "payerMno": "600600010996556", "outTradeNo": "{{outTradeNo}}", "payeeAcType": "XJH", "payerAcType": "XJH", "productInfos": [{"remark": "ullamco sunt minim Nonea nisi", "orderNo": "2131sdqwez", "productNo": "pwd2313", "orderAmount": 0.03, "productName": "标百利快元各料", "productCount": None}]}]}, "format": "UTF-8", "version": "1.2", "timestamp": "2003-02-14 13:23:21"}, "path": "/businessAction/commonBusiness/BASIC_BZFK", "query": {}, "method": "POST", "headers": {}}}
    _resolved_body = {"request": {"body": {"data": {"remark": "aaa", "notifyUrl": "http://mqhjshjkr.bm/ihb", "requestId": "OT1776263311846491868", "rootMchId": "600600010996559", "orderExpired": "90", "batchActionList": [{"amount": "3", "remark": "bbb", "payeeMno": "600600010996558", "payerMno": "600600010996556", "outTradeNo": "OT1776263311846118922", "payeeAcType": "XJH", "payerAcType": "XJH", "productInfos": [{"remark": "ullamco sunt minim Nonea nisi", "orderNo": "2131sdqwez", "productNo": "pwd2313", "orderAmount": 0.03, "productName": "标百利快元各料", "productCount": None}]}]}, "format": "UTF-8", "version": "1.2", "timestamp": "2003-02-14 13:23:21"}, "path": "/businessAction/commonBusiness/BASIC_BZFK", "query": {}, "method": "POST", "headers": {}}}
    logger.info("【请求体-原始】%s", _raw_body)
    logger.info("【请求体-变量替换后】%s", _resolved_body)
    logger.info("【请求体-变量替换】已执行变量替换，替换数=%d, 自动生成数=%d", 0, 0)
    ...
    logger.info("【完整请求】")
    logger.info("    URL: %s", BASE_URL.rstrip("/") + "/" + "/businessAction/commonBusiness/BASIC_BZFK".lstrip("/"))
    ...
    _request_url = BASE_URL.rstrip("/") + "/" + "/businessAction/commonBusiness/BASIC_BZFK".lstrip("/")
    ...
    resp = _request(
        method="POST",
        path="/businessAction/commonBusiness/BASIC_BZFK",
        headers={},
        params=None,
        json_body={"request": {"body": {"data": {"remark": "aaa", "notifyUrl": "http://mqhjshjkr.bm/ihb", "requestId": "OT1776263311846491868", "rootMchId": "600600010996559", "orderExpired": "90", "batchActionList": [{"amount": "3", "remark": "bbb", "payeeMno": "600600010996558", "payerMno": "600600010996556", "outTradeNo": "OT1776263311846118922", "payeeAcType": "XJH", "payerAcType": "XJH", "productInfos": [{"remark": "ullamco sunt minim Nonea nisi", "orderNo": "2131sdqwez", "productNo": "pwd2313", "orderAmount": 0.03, "productName": "标百利快元各料", "productCount": None}]}]}, "format": "UTF-8", "version": "1.2", "timestamp": "2003-02-14 13:23:21"}, "path": "/businessAction/commonBusiness/BASIC_BZFK", "query": {}, "method": "POST", "headers": {}}},
        timeout=30,
    )
    _extracted_data = {}
    _store_test_context("test_1_TEST_CASE_000000101", resp, None)
    ...
    logger.info("=" * 60)
    logger.info("【断言信息】")
    logger.info("    用例ID: TEST_CASE_000000101")
    logger.info("    断言数量: 0")
