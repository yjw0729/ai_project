...
# execution_id: exec-fc5c3180c261
# generated_at: 2026-04-15T20:11:13.443812
...
def test_1_TEST_CASE_000000006():
    ...
    _extracted_data = {}
    _store_test_context("test_1_TEST_CASE_000000006", resp, None)
...
def test_2_TEST_CASE_000000007():
    """
    计费
    ...
    _extracted_data = {}
    _store_test_context("test_2_TEST_CASE_000000007", resp, None)
    logger.info("【发送请求】请求已发送，等待响应...")
    logger.info("【响应信息】状态码=%s, 响应时间=%.2fms", resp.status_code, resp.elapsed.total_seconds() * 1000)
    logger.info("    响应Headers: %s", json.dumps(dict(resp.headers), ensure_ascii=False)[:500])
    logger.info("    响应体(前500字符): %s", resp.text[:500] if resp.text else "空响应")
...
def test_3_TEST_CASE_000000008():
    """
    付款

    Case ID: TEST_CASE_000000008
    Priority: P0
    Method: POST /v6/api/cashout/payment/payment
    """
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "TEST_CASE_000000008", "付款", "交易模块")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    ...
    _extracted_data = {}
    _store_test_context("test_3_TEST_CASE_000000008", resp, None)
    logger.info("【发送请求】请求已发送，等待响应...")
    logger.info("【响应信息】状态码=%s, 响应时间=%.2fms", resp.status_code, resp.elapsed.total_seconds() * 1000)
    logger.info("    响应Headers: %s", json.dumps(dict(resp.headers), ensure_ascii=False)[:500])
    logger.info("    响应体(前500字符): %s", resp.text[:500] if resp.text else "空响应")
    ...
    logger.info("=" * 60)
    logger.info("【断言信息】")
    logger.info("    用例ID: TEST_CASE_000000008")
    logger.info("    断言数量: 0")
    logger.info("    无自定义断言，使用默认状态码断言")
    logger.info("=" * 60)
    logger.info("【执行断言】开始执行断言...")
