...
# execution_id: 20260409213208_e1751325
# generated_at: 2026-04-09T21:32:08.393184
...
@allure.feature("交易模块")
@allure.story("TEST_CASE_000000005")
def test_1_TEST_CASE_000000005():
    ...
    logger.info("【请求头】%s", {"Content-Type": "application/json"})
    allure.attach('{"Content-Type": "application/json"}', name="headers", attachment_type=allure.attachment_type.JSON)
    logger.info("【Query参数】无Query参数")
    logger.info("【请求体-变量替换详情】%s", "${TIMESTAMP}->1775741528")
    ...
    _resolved_body = {"data": {"bankCode": "", "notifyUrl": "http://mqhjshjkr.bm/ihb", "payMethod": "", "requestId": "112667", "rootMchId": "600600010996929", "batchActionList": [{"amount": "30", "bindId": "", "userId": "U3141383309692437", "payeeMno": "600600010996929", "payerMno": "", "contractNo": "OTSSH1000000111", "outTradeNo": "1775741528", "productInfos": [{"remark": "Lorem ipsum dolor sit amet", "orderNo": "OTNPRODUCT0001", "productNo": "iph1234", "orderAmount": 15, "productName": "iPhone X", "productCount": 1}], "divideAccountType": "00"}]}, "format": "123123", "version": "1.2", "requestId": "708395", "timestamp": "1775741528"}
    ...
    allure.attach(json.dumps(_resolved_body, ensure_ascii=False, indent=2), name="body-resolved", attachment_type=allure.attachment_type.JSON)
    logger.info("【完整请求】")
    logger.info("    URL: %s", BASE_URL.rstrip("/") + "/" + "/businessAction/commonBusiness/BASIC_PTSD".lstrip("/"))
    logger.info("    Method: %s", "POST")
    logger.info("    Headers(替换后): %s", {"Content-Type": "application/json"})
    logger.info("    Query(替换后): %s", None)
    logger.info("    Body(替换后): %s", str(_resolved_body)[:500] if _resolved_body is not None else "None")
    with allure.step("请求信息: POST /businessAction/commonBusiness/BASIC_PTSD"):
        allure.attach('{"Content-Type": "application/json"}', name="headers", attachment_type=allure.attachment_type.JSON)
        allure.attach('{"data": {"bankCode": "", "notifyUrl": "http://mqhjshjkr.bm/ihb", "payMethod": "", "requestId": "112667", "rootMchId": "600600010996929", "batchActionList": [{"amount": "30", "bindId": "", "userId": "U3141383309692437", "payeeMno": "600600010996929", "payerMno": "", "contractNo": "OTSSH1000000111", "outTradeNo": "1775741528", "productInfos": [{"remark": "Lorem ipsum dolor sit amet", "orderNo": "OTNPRODUCT0001", "productNo": "iph1234", "orderAmount": 15, "productName": "iPhone X", "productCount": 1}], "divideAccountType": "00"}]}, "format": "123123", "version": "1.2", "requestId": "708395", "timestamp": "1775741528"}', name="body", attachment_type=allure.attachment_type.JSON)
    with allure.step("发送请求"):
        ...
        logger.info("【发送请求】完整请求: method=%s, url=%s, headers=%s, params=%s, body=%s",             "POST", _request_url, {"Content-Type": "application/json"}, None,             json.dumps(_resolved_body, ensure_ascii=False)[:500] if _resolved_body is not None else "None")
        resp = _request(
            method="POST",
            path="/businessAction/commonBusiness/BASIC_PTSD",
            headers={"Content-Type": "application/json"},
            params=None,
            json_body={"data": {"bankCode": "", "notifyUrl": "http://mqhjshjkr.bm/ihb", "payMethod": "", "requestId": "112667", "rootMchId": "600600010996929", "batchActionList": [{"amount": "30", "bindId": "", "userId": "U3141383309692437", "payeeMno": "600600010996929", "payerMno": "", "contractNo": "OTSSH1000000111", "outTradeNo": "1775741528", "productInfos": [{"remark": "Lorem ipsum dolor sit amet", "orderNo": "OTNPRODUCT0001", "productNo": "iph1234", "orderAmount": 15, "productName": "iPhone X", "productCount": 1}], "divideAccountType": "00"}]}, "format": "123123", "version": "1.2", "requestId": "708395", "timestamp": "1775741528"},
            timeout=30,
        )
