...
# execution_id: test_fix_verify
# generated_at: 2026-04-10T21:57:04.956686
...
def api_client():
    ...
    return _session
def test_1_TEST_CASE_000001():
    """
    测试描述

    Case ID: TEST_CASE_000001
    Priority: P0
    Method: POST /api/test
    """
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "TEST_CASE_000001", "测试用例", "测试模块")
    logger.info("【变量配置】从 preconditions 加载 1 个变量: %s", "{variables_json}")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/api/test", BASE_URL.rstrip("/") + "/" + "/api/test".lstrip("/"))
    ...
    logger.info("【完整请求】")
    logger.info("    URL: %s", BASE_URL.rstrip("/") + "/" + "/api/test".lstrip("/"))
    ...
    _request_url = BASE_URL.rstrip("/") + "/" + "/api/test".lstrip("/")
    ...
    logger.info("=" * 60)
    logger.info("【断言信息】")
    logger.info("    用例ID: TEST_CASE_000001")
    logger.info("    断言数量: 0")
