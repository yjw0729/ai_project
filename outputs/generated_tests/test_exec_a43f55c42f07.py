...
# execution_id: exec-a43f55c42f07
# generated_at: 2026-04-15T19:51:36.457138
...
def test_1_DB_0():
    ...
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "DB_0", "test_case_1", "API测试")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/", BASE_URL.rstrip("/") + "/" + "/".lstrip("/"))
    ...
    logger.info("【完整请求】")
    logger.info("    URL: %s", BASE_URL.rstrip("/") + "/" + "/".lstrip("/"))
    logger.info("    Method: %s", "GET")
    ...
    _request_url = BASE_URL.rstrip("/") + "/" + "/".lstrip("/")
    logger.info("【发送请求】即将发送: %s %s", "GET", _request_url)
    logger.info("【发送请求】完整请求: method=%s, url=%s, headers=%s, params=%s, body=%s", "GET", _request_url, {}, None, json.dumps(_resolved_body, ensure_ascii=False)[:500] if _resolved_body is not None else "None")
    resp = _request(
        method="GET",
        path="/",
        headers={},
    ...
    _extracted_data = {}
    _store_test_context("test_1_DB_0", resp, None)
...
def test_2_DB_1():
    ...
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "DB_1", "test_case_2", "API测试")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/", BASE_URL.rstrip("/") + "/" + "/".lstrip("/"))
    logger.info("【请求头】无自定义请求头，使用默认Content-Type")
    ...
    _extracted_data = {}
    _store_test_context("test_2_DB_1", resp, None)
    logger.info("【发送请求】请求已发送，等待响应...")
    logger.info("【响应信息】状态码=%s, 响应时间=%.2fms", resp.status_code, resp.elapsed.total_seconds() * 1000)
    logger.info("    响应Headers: %s", json.dumps(dict(resp.headers), ensure_ascii=False)[:500])
    logger.info("    响应体(前500字符): %s", resp.text[:500] if resp.text else "空响应")
...
def test_3_DB_2():
    ...
    logger.info("=" * 60)
    logger.info("【用例开始】%s | %s", EXECUTION_ID, __name__)
    logger.info("【用例信息】ID=%s, 名称=%s, 模块=%s", "DB_2", "test_case_3", "API测试")
    logger.info("【变量配置】无 preconditions 变量配置")
    logger.info("=" * 60)
    logger.info("【请求地址】base_url=%s, 接口路径=%s, 完整URL=%s", BASE_URL, "/", BASE_URL.rstrip("/") + "/" + "/".lstrip("/"))
    logger.info("【请求头】无自定义请求头，使用默认Content-Type")
    ...
    _extracted_data = {}
    _store_test_context("test_3_DB_2", resp, None)
    logger.info("【发送请求】请求已发送，等待响应...")
    logger.info("【响应信息】状态码=%s, 响应时间=%.2fms", resp.status_code, resp.elapsed.total_seconds() * 1000)
    logger.info("    响应Headers: %s", json.dumps(dict(resp.headers), ensure_ascii=False)[:500])
    logger.info("    响应体(前500字符): %s", resp.text[:500] if resp.text else "空响应")
