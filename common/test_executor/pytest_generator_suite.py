...
class PytestGeneratorSuite:
    ...
    def _build_one_test_function(
        ...
        case_id = self._get(case, "case_id") or f"DB_{self._get(case, 'id') or idx}"
        name = self._get(case, "name") or f"test_case_{idx + 1}"
        method = (self._get(case, "method") or "GET").upper()
        path = self._get(case, "path") or "/"
        priority = self._get(case, "priority") or "P2"
        description = self._get(case, "description") or ""
        timeout = self._get(case, "timeout") or 30

        headers = self._get(case, "headers") or {}
        query_params = self._get(case, "query_params") or {}
        request_body = self._get(case, "request_body")
        assertions = self._get(case, "assertions") or []
        # 数据库断言配置
        post_script = self._get(case, "post_script") or []
        db_checks = self._get(case, "db_checks") or []
        extract_fields = self._get(case, "extract_fields") or []
        module = self._get(case, "module") or "API测试"

        # ========== 变量替换阶段 ==========
        # 获取用例的 preconditions 变量
        case_variables = self._get(case, 'case_variables') or {}
