...
class AssertionGenerator:
    ...
    def generate_from_api_spec(
        ...
        Args:
            api_spec: API规范，包含 responses 字段
    ...
    def _generate_json_path_assertions(
        self,
        body: Any,
        path_prefix: str = "$",
    ) -> List[Dict[str, Any]]:
        ...
        if isinstance(body, dict):
            for key, value in body.items():
                ...
                if key in ("code", "status", "ret", "resultCode", "errorCode"):
                ...
                elif key in ("message", "msg", "msg", "errorMessage", "errorMsg"):
                    assertions.append({
                        "type": "contains",
                        "expected": "success",
                        "path": current_path,
                    ...
                    nested_assertions = self._generate_json_path_assertions(
                        value, current_path
                    )
                    ...
                    if isinstance(first_item, dict):
                        ...
                        nested_assertions = self._generate_json_path_assertions(
                            first_item, item_path
                        )
                        assertions.extend(nested_assertions)

        return assertions

    def _generate_contains_assertions(
        self,
        body: Any,
    ...
    def generate_assertions_for_test_case(
        ...
        request_block = test_data.get("request", {})
        expected = test_case.get("expect", "")

        assertions.append({
            "type": "status_code",
            "expected": test_case.get("status_code", 200),
        })
...
class SmartAssertionGenerator(AssertionGenerator):
    ...
    def generate_smart_assertions(
        ...
        Returns:
            List[Dict[str, Any]]: 断言列表
        """
        base_assertions = self.generate_from_response(
        ...
        try:
            ai_assertions = self._generate_ai_assertions(
                response, context
            )
    ...
    def _generate_ai_assertions(
        ...
        try:
            ...
            assertions = self._parse_ai_response(llm_response)
            return assertions
        except Exception as e:
...
def generate_assertions(
    ...
    return generator.generate_from_response(
        response,
        response_time_ms,
