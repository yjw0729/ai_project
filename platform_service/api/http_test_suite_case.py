    ...
    try:
        ...
        for i, case_id in enumerate(case_ids):
            try:
                ...
                if existing:
                    failed_cases.append({"case_id": case_id, "reason": "用例已存在于该套件"})
                    continue
                ...
                if inherit_from_case:
                    # 继承用例配置：自动从 test_case 复制 preconditions/test_steps/test_data
...
def update_suite_case(suite_id: int, suite_case_id: int):
    ...
    try:
        ...
        if payload.get("reset_to_inherit"):
            suite_case.clear_request_config()
            logger.info(f"[TestSuiteCase] 重置用例配置: suite_case_id={suite_case_id}")
            mapper.update(suite_case_id, {"id": suite_case_id})  # 触发更新
            return _json_response({
                "code": 200,
                "message": "已重置为继承用例配置",
        ...
        return _json_response({
            ...
            "data": {
                "id": updated.id,
                "execution_order": updated.execution_order,
                "enabled": updated.enabled,
                "has_custom_config": updated.has_custom_config(),
                "custom_config_summary": updated.get_custom_config_summary()
            }
        })
